...
- **L4. Stale `docker-compose.yml`** in the repo serves `TinyLlama`, but production serves `Qwen/Qwen3-8B`.
  Sync it so the committed compose matches reality. ✅ RESOLVED (2026-06-18) — committed compose is now a
  faithful mirror of the actual production file pulled from the server (`~/qwen3/docker-compose.yml`):
  `Qwen/Qwen3-8B` served as `qwen3-8b`, bitsandbytes quantization + load-format, `--dtype float16`,
  `--max-model-len 16384`, `--gpu-memory-utilization 0.90`, `--max-num-seqs 32`, `--enforce-eager`,
  `--trust-remote-code`, `runtime: nvidia`, `restart: unless-stopped`, `HF_HOME`, and the `/health`
  healthcheck. No longer guessed — verified against the running deployment.
